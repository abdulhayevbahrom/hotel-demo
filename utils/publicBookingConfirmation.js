...
const getWebsiteUrl = () =>
  String(
    process.env.PUBLIC_WEBSITE_URL || "https://demo.my-hotels.uz",
  ).replace(/\/+$/, "");
const escapeHtml = (value) => String(value || "").replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[character]));

const getTransporter = () => {
  const user = process.env.BOOKING_SMTP_USER || "hotel.demo@mail.ru";
...
const sendBookingConfirmationEmail = async ({ email, guestName, reference, token }) => {
  ...
  const from = process.env.BOOKING_MAIL_FROM || `Demo Hotel <${process.env.BOOKING_SMTP_USER || "hotel.demo@mail.ru"}>`;
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#172033"><h2>Demo Hotel</h2><p>Assalomu alaykum, <strong>${safeGuestName}</strong>.</p><p>Broningiz muvaffaqiyatli qabul qilindi.</p><p>Bron raqami: <strong>${safeReference}</strong></p><p><a href="${confirmationUrl}" style="display:inline-block;padding:13px 20px;border-radius:8px;background:#bb8b3d;color:#fff;text-decoration:none;font-weight:700">Bron qog'ozini ochish</a></p><p>Ushbu havola orqali bron ma'lumotlarini ko'rishingiz va PDF yuklab olishingiz mumkin.</p></div>`,
  });
...
const writeBookingPdf = ({ res, booking }) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="demo-booking-${booking.reference}.pdf"`);
  doc.pipe(res);
  doc.fontSize(24).fillColor("#0e5b76").text("DEMO HOTEL", { align: "center" });
  ...
  doc.moveDown(2).fillColor("#536179").font("Helvetica").fontSize(9).text("Demo Hotel | test.hotel@mail.ru", { align: "center" });
  doc.end();
};

...
const hasRoomStayConflict = async ({ roomId, stayStart, stayEnd }) => {
  ...
  return guests.some((guest) => {
    const guestStart =
      guest.status === "booked" ? guest.bookedForAt : guest.checkInAt;
...
const serializePublicBooking = (guests) => {
  ...
  publicCheckOut.setUTCDate(
    publicCheckOut.getUTCDate() + Math.max(Number(primary.stayDays || 1), 1),
  );
  return {
    ...
    rooms: guests.map((guest) => ({
      ...
      roomNumber: guest.room
        ? `${guest.room.korpus ? `${guest.room.korpus}-` : ""}${guest.room.roomNumber || ""}`
        : "",
      dailyRate: guest.dailyRate || 0,
      stayDays: guest.stayDays || 1,
    })),
    totalAmount: guests.reduce(
...
const getPublicBookingConfirmation = async (req, res) => {
  try {
    ...
    if (!booking)
      return response.notFound(res, "Bron topilmadi yoki havola yaroqsiz");
  ...
  try {
    ...
    if (!checkIn || !checkOut || checkOut.getTime() <= checkIn.getTime()) {
      return response.error(
        res,
        "To'g'ri kelish va ketish sanalarini kiriting",
      );
    }
    ...
    const guests = await Guest.find({
      ...
      .select("room status bookedForAt checkInAt checkoutDueAt")
      .lean();
...
const createPublicBooking = async (req, res) => {
  try {
    ...
    if (checkIn.getTime() < todayStart.getTime()) {
      return response.error(
        res,
        "Kelish sanasi hozirgi vaqtdan oldin bo'lishi mumkin emas",
      );
    }
    ...
    if (bookingRequestId) {
      const fingerprintPayload = JSON.stringify({
        ...
        selections: requestedSelections
          .map(({ category, capacity, rate, count }) => ({
            category,
            capacity,
            rate,
            count,
          ...
          .sort((a, b) =>
            `${a.category}-${a.capacity}-${a.rate}`.localeCompare(
              `${b.category}-${b.capacity}-${b.rate}`,
            ),
          ),
      ...
      const fingerprint = crypto
        ...
        .digest("hex");
      try {
        await PublicBookingRequest.create({
        ...
        if (error?.code === 11000) {
          return response.error(
            res,
            "Bu bron so'rovi allaqachon qabul qilingan",
          );
        }
        throw error;
      }
    ...
    for (const selection of selections) {
      ...
      if (selection.capacity > 0) roomFilter.capacity = selection.capacity;
        ...
        roomFilter.capacity = { $gte: guestsCount };
      if (selection.rate > 0) {
        roomFilter[
          `prices.${guestType === "chetellik" ? "chetEllik" : "oddiy"}`
        ] = selection.rate;
      }
      ...
      if (found < selection.count) {
        return response.error(
          res,
          `${selection.category} uchun yetarli bo'sh xona topilmadi`,
        );
      }
    }
    ...
    if (selectedRooms.length < roomCount || totalCapacity < guestsCount) {
      return response.error(
        res,
        "Tanlangan muddat uchun mos bo'sh xona topilmadi",
      );
    }
    ...
    try {
      ...
      console.error(
        "Bron tasdiq emailini yuborib bo'lmadi:",
        emailError.message,
      );
    }
    return response.created(res, "Bron qabul qilindi", {
      ...
      confirmationUrl: `${String(process.env.PUBLIC_WEBSITE_URL || "https://demo.my-hotels.uz").replace(/\/+$/, "")}/booking-confirmation/${publicToken}`,
